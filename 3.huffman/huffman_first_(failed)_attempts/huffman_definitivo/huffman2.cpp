...
int huffman_compressor(std::ifstream& is, std::ofstream& os) {
	...
	for (const auto& code : v[0].second.v_) {
		table[code->sym_] = h_code(code->code_, code->len_);
		table_print.push_back({ code->sym_, code->len_ });
	}
	std::sort(begin(table_print), end(table_print), [](const pair<char, uint32_t>& a1, const pair<char, uint32_t>& a2) {
	...
	while (total_counter-->0) {
		char val = is.get();
		bw.write_n_bits(table[val].code_, table[val].len_);
	}
