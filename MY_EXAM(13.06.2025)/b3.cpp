...
template<typename T>
T read_number(std::ifstream& is, T& val, uint8_t nbytes) {
	T val = 0;
...
bool decompress(std::ifstream& is, std::ofstream& os) {
	...
	read_number(is, creation_date, 4);
	...
	read_number(is, compression, 2);
	is.ignore(2);
	uint32_t text_len;
	read_number(is, text_len, 4);
	uint16_t record_count;
	read_number(is, record_count, 2);
	uint16_t record_size;
	read_number(is, record_size, 2);
	uint16_t encryption_type;
	read_number<uint16_t>(is, encryption_type, 2);
	...
	for (int c = 1; c <= record_count; c++) {
		...
		while (decoded_data.size() < record_size and !stop_decoding_record) {
